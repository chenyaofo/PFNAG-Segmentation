...
class SPOSMobileNetV3Segmentation(nn.Module):
    ...
    def __init__(self, n_classes=19, bn_param=(0.1, 1e-5), dropout_rate=0.1, base_stage_width=None, width_mult=1.0,
        ...
        for width, n_block, s, act_func, use_se, dilated in zip(width_list[2:], n_block_list[1:],
                ...
                for ks in self.ks_list:
                    for r in self.expand_ratio_list:
                        mobile_inverted_conv = MBConvLayer(
                            ...
                            act_func=act_func, use_se=use_se, dilated=dilated
                        )
        ...
        self.head = DeepLabHead(feature_dim, n_classes)
    ...
    def get_subnet(self, arch):
        ...
        for stage_id, block_idx in enumerate(self.block_group_info):
            ...
            for idx in active_idx:
                c_ks, c_r = arch.ks[idx], arch.ratios[idx]
                idx_layer = self.ks_list.index(c_ks) * len(self.ks_list) + self.expand_ratio_list.index(c_r)
                if stage_id <= split_id:
                    blocks.append(copy.deepcopy(self.blocks[idx][idx_layer]))
                else:
                    remain_block.append(copy.deepcopy(self.blocks[idx][idx_layer]))

        aux_channels = blocks[-1].conv.point_linear._modules["conv"].out_channels

        aux_head = DeepLabHead(aux_channels, self.n_classes)
        _subnet = MobileNetV3Segmentation(first_conv, blocks, remain_block, head, aux_head)
